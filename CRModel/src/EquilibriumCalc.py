...
def SahaRelation_V(p_0,T_e,T_g,Q_n,Q_i):
    ...
    # We assume constant volume here. n_0 = n_neutral + n_e 
...
def BoltzmannDistribution(n_neutral,T_e,Qtot,E_lvl,g_lvl):
    ...
    for i in range(q):
        if i == 0:
            npop_LTE[i] = g_lvl[i]/Qtot*n_neutral
            continue    
        npop_LTE[i] = g_lvl[i]*np.exp(-E_lvl[i]*cm_eV/T_e)/Qtot*n_neutral
