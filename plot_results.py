...
# load solution file
D = np.load('newton_4spec_Np150_fullsoln_AllSamples.npy')
D = np.transpose(D)
...
phi = np.zeros((Np,T+1))
tds = cs.timeDomainCollocationSolver(4,1,Np,scenario=2)
...
plt.setp(cbar.ax.set_yticklabels(), rotation='horizontal', fontsize=12)
...
plt.savefig('Te_phi_4spec_line.pdf')
...
plt.setp(ax2.get_yticklabels(), fontsize=12)
plt.savefig('Te_phi_4spec_mean.pdf')
