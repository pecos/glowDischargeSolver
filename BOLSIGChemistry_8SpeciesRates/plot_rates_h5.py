...
row_temp = []
data = []
header = ["", "Te(eV)", "", "ND(1/m-s)", ""]
for m in range(len(Te_trans)):
    row_temp.append('(')
    row_temp.append(Te_trans[m])
    row_temp.append(',')
    row_temp.append(diff[m])
    row_temp.append(')')
    data.append(row_temp)
    row_temp = []

zz = open("./CSVs/Diffusivity.csv", 'w')
writer = csv.writer(zz)
writer.writerow(header)
writer.writerows(data)
zz.close()
